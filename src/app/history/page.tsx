      ...
      <div className="flex flex-col gap-8 2xl:flex-row">
        {recentHistory.length ? (
          recentHistory.map((auction) => (
            <div
              key={auction.id}
              className="flex flex-1 flex-col gap-4 rounded-2xl bg-gradient-to-b from-slate-900 to-slate-950 px-4 py-8 text-slate-200 shadow-lg dark:from-slate-900/50 dark:to-slate-950/50"
            >
              ...
              <ScrollArea className="px-4">
                <ul className="flex h-[800px] max-h-[55dvh] flex-col gap-4">
                  {auction.items.map(
                    (item, index) =>
                      item && (
                        <li key={index}>
                          ...
                          >
                            <div className="flex items-center gap-4 rounded-lg bg-slate-800 p-2 pr-4 text-xl font-medium shadow-lg transition-colors hover:bg-slate-800/50 2xl:text-sm">
                              ...
                              />
                              <span className="line-clamp-2 flex-1 text-purple-600">
                                {item.name}
                              </span>
          ...
          <span className="text-xl text-red-600">No History found!</span>
        )}
      </div>
    </section>
  )
